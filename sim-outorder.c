...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-bpred",
		 "branch predictor type {nottaken|taken|perfect|bimod|2lev|comb|tbit}",
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_string(odb, "-cache:trc",
		 "TRC cache config, i.e., {<config>|none}",
		 &trc_opt, "trc:2048:512",
		 /* print */TRUE, NULL);
...
{
  char name[128], c;
  int nsets, bsize, assoc, htbsize, tetsize;
  ...
  else if (!mystricmp(pred_type, "tbit"))
  {
    tbit_size = 128; 
    /* bimod with tbit */
    if (bimod_nelt != 1)
  ...
  pred = bpred_create(BPredTbit,
    ...
    //pred = bpred_create(BPredTbit, 0, 0, 0, 0, 0, 0, 0, 0, 0, tbit_size);
  }
  else
  ...
  /* use a TRC? */
  if (!mystricmp(trc_opt, "none"))
    cache_trc = NULL;
  else
    {
      if (sscanf(trc_opt, "%[^:]:%d:%d", name, &htbsize, &tetsize) != 3 )
	fatal("bad TRC params: <name>:<htb_size>:<tet_size>");
      cache_trc = create_trc(htbsize, tetsize);
    }
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  if(cache_trc)
    cache_reg_stats_trc(cache_trc,sdb);
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
	  ...
	  && (MD_OP_FLAGS(rs->op) & F_CTRL))
	{
      ...
      if(cache_trc)
	insert_htb(cache_trc, rs->PC, rs->IR, rs->next_PC != (rs->PC + sizeof(md_inst_t)) );
       ...
       i++)
  {
    ...
    if( cache_trc )
    {
      if( cache_trc->trc_mode )
	    {
	      if( cache_trc->htb->table[cache_trc->htb_pc].addr != fetch_regs_PC || cache_trc->htb_pc == cache_trc->htb->index )
	      {
	         cache_trc->trc_mode = 0;
	         cache_trc->misses++;
	         goto cachemodecycle;
	      }
	      else 
        {
	         inst = cache_trc->htb->table[cache_trc->htb_pc].inst;
	         cache_trc->htb_pc += 1;
	         cache_trc->hits++;
           trc_access++;
	         goto trcmodecycle;
	   ...
	   {
	     cache_trc->tetaccesses++;
	     if( ( cache_trc->tet->table[TET_INDEX(cache_trc, fetch_regs_PC)].tag == TET_TAG(cache_trc, fetch_regs_PC) ) && cache_trc->tet->table[TET_INDEX(cache_trc, fetch_regs_PC)].busy )
	     {
	       cache_trc->htb_pc = cache_trc->tet->table[TET_INDEX(cache_trc, fetch_regs_PC)].htbindex;
	       cache_trc->trc_mode = 1;
	     }
	    }
    }
    ...
    if (pred)
	 {
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL)
	    ...
	    fetch_pred_PC = 0;
	  // fetch_data[fetch_tail].dir_update.dir.bimod will contain if bimod is used
	  dir_bimod = fetch_data[fetch_tail].dir_update.dir.bimod;
