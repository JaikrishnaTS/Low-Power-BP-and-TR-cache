...
/* an entry in a TBIT */
struct bpred_tbit_ent_t {
  md_addr_t tag;		/* address of the branch being tracked */
  unsigned char tc;		/* taken counter */
  unsigned char at; 		/* always taken bit */
  md_addr_t target;		/* target of the branch */
};
...
struct bpred_t {
  ...
  struct {
    int size;			/* num TBIT entries */
    struct bpred_tbit_ent_t *tbit_data; /* TBIT addr-prediction table */
  } tbit;
  ...
  counter_t used_tbit;		/* num TBIT predictions used */
  ...
  counter_t tbit_hits;		/* num tbit hits */
