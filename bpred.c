...
bpred_create(enum bpred_class class,	/* type of predictor to create */
	     ...
	     unsigned int tbit_size)	/* num entries in TBIT */
{
  struct bpred_t *pred;
  ...
  case BPredTbit:
    pred->dirpred.bimod = 
      bpred_dir_create(class, bimod_size, 0, 0, 0);
    pred->tbit.size = tbit_size;
    if (!(pred->tbit.tbit_data = calloc(pred->tbit.size , sizeof(struct bpred_tbit_ent_t))))
      fatal("cannot allocate TBIT");
    break;
  default:
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredTbit:
    fprintf(stream, "pred_dir: %s: entries: 128", name);
  case BPred2bit:
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  case BPredTbit:
    bpred_dir_config(pred->dirpred.bimod, "tbit", stream);
    fprintf(stream, "btb: %d sets x %d associativity", 
	    pred->btb.sets, pred->btb.assoc);
    fprintf(stream, "ret_stack: %d entries", pred->retstack.size);
    break;
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  else if(pred->class == BPredTbit)
  {
    sprintf(buf, "%s.used_tbit", name);
    stat_reg_counter(sdb, buf,"total number of TBIT predictions used", &pred->used_tbit, 0, NULL);
    sprintf(buf, "%s.tbit_hits", name);
    stat_reg_counter(sdb, buf,"total number of tbit hits", &pred->tbit_hits, 0, NULL);
  }
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
    ...
    case BPredTbit:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
      {
	md_addr_t tt = ((baddr & 0x3FC00) >> 10) ^ ((baddr & 0xFF00000) >> 20);
	unsigned char ind = (baddr & 0x1FC) >> 2;
	if ( pred->tbit.tbit_data[ind].tag == tt)
	{
	  dir_update_ptr->dir.tbit = TRUE;
	  if( pred->tbit.tbit_data[ind].at )
	  {
	    dir_update_ptr->tbittarget = pred->tbit.tbit_data[ind].target;
	  }
	  else
    {
  ...
  if(pred->class == BPredTbit)
    if( dir_update_ptr->dir.tbit && pred->tbit.tbit_data[(baddr & 0x1FC) >> 2].at )
      return dir_update_ptr->tbittarget;
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if ( pred->class == BPredTbit )
  {
    ...
    {
      if(dir_update_ptr->dir.tbit) // tbit hit . not necessarily correct
      {
	if( pred->tbit.tbit_data[ind].at ) // if at == 1
	{
	  if ( pred->tbit.tbit_data[ind].tc < 3 )
	    pred->tbit.tbit_data[ind].tc++;
	  return;
	}
	else // if at != 1
	{
	  pred->tbit.tbit_data[ind].tc++;
	  if( pred->tbit.tbit_data[ind].tc == 3 ) // if tc == 3
	  {
	    pred->tbit.tbit_data[ind].at = 1;
	    return;
	  } // else execute update normally
	}
      }
      else // entry not in tbit
      {
	md_addr_t tt = ((baddr & 0x3FC00) >> 10) ^ ((baddr & 0xFF00000) >> 20);
	pred->tbit.tbit_data[ind].tag = tt;
	pred->tbit.tbit_data[ind].target = btarget;
      }
    }
    else // not taken
    {
	...
	if ( pred->tbit.tbit_data[ind].at ) // if at == 1
	{
	  if ( pred->tbit.tbit_data[ind].tc == 3 ) // tc == 3
	  {
	    pred->tbit.tbit_data[ind].tc = 0;
	    // execute update normally
	  }
	  else // tc != 3
	  {
	    pred->tbit.tbit_data[ind].tag = 0;
	    // execute update normally
	  }
	}
	else // at != 1
