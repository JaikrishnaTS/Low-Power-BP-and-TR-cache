...
//#define TET_TAG(addr) (((addr >> 24) ^ (addr >> 16) ^ (addr >> 8) ^ (addr)) & 0xFF)
#define TET_TAG(t, addr) (addr >> (log_base2(sizeof(md_inst_t)) + log_base2(t->tet->size)))
#define TET_INDEX(t, addr) ((addr >> log_base2(sizeof(md_inst_t))) & (t->tet->size - 1))
...
/* structures for TRC implementation */

struct tet_ent_t {
  byte_t busy;
  md_addr_t tag;
  unsigned int htbindex;
};
...
struct htb_t {
  ...
  unsigned int index;
  // unsigned int tail;
  struct htb_ent_t* table;
};

struct tet_t {
  unsigned int size;
  struct tet_ent_t* table;
};
...
struct trc {
  ...
  unsigned int htb_pc;
  byte_t trc_mode;
  counter_t misses, hits, tetaccesses;
};
