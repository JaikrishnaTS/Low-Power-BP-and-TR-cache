...
extern int tbit_size;

extern int dir_bimod;
...
static counter_t total_trc_access=0;
...
static counter_t max_trc_access;
...
void update_power_stats()
{
  ...
  if (tbit_size==128) {
    if(dir_bimod==TRUE) {
      ...
      bpred_power+=power.tbit;
      //printf("dir_mod=true\n");
    } else {
      bpred_power+=power.tbit;
      //printf("dir_mod=false\n");
    }
  } else {
    bpred_power+=power.bpred_power;
    //printf("bpred updated\n");
  }
  ...
  max_trc_access=MAX(trc_access,max_trc_access);
  ...
  if(bpred_access) {                         //HERE!!!!!!!!!!!!!!!!!
    if(bpred_access <= 2)
      {
        bpred_power_cc1+=power.tbit;
        if(dir_bimod==TRUE)
        {
    ...
    else
      {
        ...
        bpred_power_cc1+=((double)bpred_access/2.0)*power.tbit;
        if(dir_bimod==TRUE)
        {
          bpred_power_cc1+=((double)bpred_access/2.0)*power.bpred_power;
        }
      }
    bpred_power_cc2+=((double)bpred_access/2.0) * power.bpred_power; //tbit?
    bpred_power_cc3+=((double)bpred_access/2.0) * power.bpred_power; //tbit?
  }
  else
    bpred_power_cc3+=turnoff_factor*(power.bpred_power+power.tbit);
  ...
  total_cycle_power = rename_power + bpred_power + window_power + 
    lsq_power + regfile_power + trc_power + icache_power + dcache_power +
...
power_reg_stats(struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  stat_reg_double(sdb, "trc_power", "total power usage of trc", &trc_power, 0, NULL);
  ...
  stat_reg_formula(sdb, "avg_trc_power", "avg power usage of trc", "trc_power/sim_cycle",  NULL);
  ...
  stat_reg_formula(sdb, "fetch_stage_power", "total power usage of fetch stage", "icache_power + trc_power + bpred_power", NULL);
  ...
  stat_reg_formula(sdb, "avg_fetch_power", "average power of fetch unit per cycle", "(icache_power + trc_power + bpred_power)/ sim_cycle", /* format */NULL);
  ...
  stat_reg_formula(sdb, "total_power", "total power per cycle","(rename_power + bpred_power + window_power + lsq_power + regfile_power + icache_power  + trc_power + resultbus_power + clock_power + alu_power + dcache_power + dcache2_power)", NULL);

  stat_reg_formula(sdb, "avg_total_power_cycle", "average total power per cycle","(rename_power + bpred_power + window_power + lsq_power + regfile_power + icache_power + trc_power + resultbus_power + clock_power + alu_power + dcache_power + dcache2_power)/sim_cycle", NULL);

  stat_reg_formula(sdb, "avg_total_power_cycle_nofp_nod2", "average total power per cycle","(rename_power + bpred_power + window_power + lsq_power + regfile_power + icache_power + trc_power + resultbus_power + clock_power + alu_power + dcache_power - falu_power )/sim_cycle", NULL);

  stat_reg_formula(sdb, "avg_total_power_insn", "average total power per insn","(rename_power + bpred_power + window_power + lsq_power + regfile_power + icache_power + trc_power + resultbus_power + clock_power + alu_power + dcache_power + dcache2_power)/sim_total_insn", NULL);

  stat_reg_formula(sdb, "avg_total_power_insn_nofp_nod2", "average total power per insn","(rename_power + bpred_power + window_power + lsq_power + regfile_power + icache_power + trc_power + resultbus_power + clock_power + alu_power + dcache_power - falu_power )/sim_total_insn", NULL);
  ...
  stat_reg_counter(sdb, "total_trc_access", "total number accesses of trc", &total_trc_access, 0, NULL);
  ...
  stat_reg_formula(sdb, "avg_trc_access", "avg number accesses of trc", "total_trc_access/sim_cycle",  NULL);
  ...
  stat_reg_counter(sdb, "max_trc_access", "max number accesses of icache", &max_trc_access, 0, NULL);
     ...
     power_result_type *power;
{
  ...
  bpred_power = power->btb + power->local_predict + power->global_predict + 
    power->chooser + power->ras +power->tbit;
  ...
  total_power = bpred_power + rename_power + window_power + regfile_power +
    power->resultbus + lsq_power + trc_power +
  ...
  fprintf(stderr," tet_power (W): %g\n",power->tet);
  fprintf(stderr," htb_power (W): %g\n",power->htb);
  fprintf(stderr," trc_power (W): %g\n",power->trc);
...
time_parameters.cache_size = tbit_size * (data_width/8) * 1; /* C */  //configure TBIT HERE!!
  time_parameters.block_size = (data_width/8); /* B */
  time_parameters.associativity = 1; /* A */
  time_parameters.number_of_sets = tbit_size; /* C/(B*A) */
  ...
  power->tbit=array_decoder_power(rowsb,colsb,predeclength,1,1,cache) + array_wordline_power(rowsb,colsb,wordlinelength,1,1,cache) + array_bitline_power(rowsb,colsb,bitlinelength,1,1,cache) + senseamp_power(colsb);
...
//////////----------------------------------------------------------------------///////////////////////////////////////
  ...
  power->btb = ndwl*ndbl*(array_decoder_power(rowsb,colsb,predeclength,1,1,cache) + array_wordline_power(rowsb,colsb,wordlinelength,1,1,cache) + array_bitline_power(rowsb,colsb,bitlinelength,1,1,cache) + senseamp_power(colsb)); //HERE!!!!!!!!!!
...
if(cache_trc)
{
time_parameters.cache_size = cache_trc->htb->size * 8 * 1; /* C */  //configure TBIT HERE!!
  time_parameters.block_size = 8; /* B */
  time_parameters.associativity = 1; /* A */
  time_parameters.number_of_sets = cache_trc->htb->size; /* C/(B*A) */
  ...
  power->htb=array_decoder_power(rowsb,colsb,predeclength,1,1,cache) + array_wordline_power(rowsb,colsb,wordlinelength,1,1,cache) + array_bitline_power(rowsb,colsb,bitlinelength,1,1,cache) + senseamp_power(colsb);
  ...
  time_parameters.cache_size = cache_trc->tet->size * (1+ floor((1+30+(int)logtwo(cache_trc->htb->size))/8)) * 1; /* C */  //configure TBIT HERE!!
  time_parameters.block_size = 1+floor((1+30+(int)logtwo(cache_trc->htb->size))/8); /* B */
  time_parameters.associativity = 1; /* A */
  time_parameters.number_of_sets = cache_trc->tet->size; /* C/(B*A) */
  ...
  power->tet=array_decoder_power(rowsb,colsb,predeclength,1,1,cache) + array_wordline_power(rowsb,colsb,wordlinelength,1,1,cache) + array_bitline_power(rowsb,colsb,bitlinelength,1,1,cache) + senseamp_power(colsb);

  power->trc=power->tet+power->htb;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  power->total_power = power->local_predict + power->global_predict +  // total power if all components are used in one cycle
    power->chooser + power->btb + power->tbit +
  ...
  power->bpred_power = power->btb + power->local_predict + power->global_predict + power->chooser + power->ras; //HERE!!!!!!!!!!
