...
struct trc * create_trc(unsigned int htb_size, unsigned int tet_size) {
  if (htb_size <= 0)
    fatal("htb size `%d' must be non-zero", htb_size);
  if ((htb_size & (htb_size-1)) != 0)
    fatal("htb size `%d' is not a power of two", htb_size);
  if (tet_size <= 0)
    fatal("tet size `%d' must be non-zero", tet_size);
  if ((tet_size & (tet_size-1)) != 0)
    fatal("tet size `%d' is not a power of two", tet_size);
  struct trc* tt = (struct trc *) calloc(1,sizeof(struct trc));
  if(!(tt))
    fatal("cannot allocate trc");
  if(!(tt->htb = calloc(1,sizeof(struct htb_t))))
    fatal("cannot allocate htb");
  if(!(tt->tet = calloc(1,sizeof(struct tet_t))))
    fatal("cannot allocate tet");

  tt->htb->size = htb_size;
  tt->tet->size = tet_size;
  tt->htb->index = 0;
  tt->trc_mode = 0;
  tt->hits = 0;
  tt->misses = 0;
  ...
  if(!(tt->htb->table = calloc(htb_size, sizeof(struct htb_ent_t))))
    fatal("cannot allocate htb entries");
  if(!(tt->tet->table = calloc(tet_size, sizeof(struct tet_ent_t))))
    fatal("cannot allocate tet entries");
  return tt; // or just h?
}

void insert_htb(struct trc* t, md_addr_t addr, md_inst_t inst, byte_t taken) {
  enum md_opcode opc;
  if( t->htb->table[t->htb->index].inv )
    t->tet->table[TET_INDEX(t, t->htb->table[t->htb->index].addr)].busy = 0;
    // calculate appropriate tet index from t->htb->table[t->htb->index].addr and set busy to 0

  t->htb->table[t->htb->index].addr = addr;
  t->htb->table[t->htb->index].inst = inst;
  
  
  MD_SET_OPCODE(opc, inst);
  
  if( MD_OP_FLAGS(opc) & F_CTRL ) {
    //t->htb->table[t->htb->index].taken = taken;
    unsigned int tet_index = TET_INDEX(t, addr); // >> log_base2(sizeof(md_inst_t)) & (t->tet->size - 1);

    if(t->tet->table[tet_index].busy == 0) {
      t->tet->table[tet_index].tag = TET_TAG(t, addr);
      t->tet->table[tet_index].htbindex = (t->htb->index+1) % t->htb->size;
      t->tet->table[tet_index].busy = 1;
      t->htb->table[t->htb->index].inv = 1;
    }
    else {
      t->htb->table[t->htb->index].inv = 0;
    }
  }
  else {
    t->htb->table[t->htb->index].inv = 0;
  }
  t->htb->index = (t->htb->index + 1) % t->htb->size;
}
...
void
cache_reg_stats_trc(struct trc *tt,	/* cache instance */
		struct stat_sdb_t *sdb)	/* stats database */
{
  char buf[512], buf1[512], *name;
  ...
  stat_reg_formula(sdb, buf, "total number of accesses", buf1, "%12.0f");
  sprintf(buf, "%s.hits", name);
  stat_reg_counter(sdb, buf, "total number of hits", &tt->hits, 0, NULL);
  sprintf(buf, "%s.misses", name);
  stat_reg_counter(sdb, buf, "total number of misses", &tt->misses, 0, NULL);
  sprintf(buf, "%s.tetaccesses", name);
  stat_reg_counter(sdb, buf, "total number of TET accesses",
		 &tt->tetaccesses, 0, NULL);
}
